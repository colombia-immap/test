...
if len(sys.argv) < 3:
    ...
    print("Examples:")
    print("-- Retrieve all the cities in Peru: %s BR city" % (sys.argv[0]))
...
for i in range(result.shape[0]):
    result.values[i].encode('utf-8')
    a += print_json(result.values[i], location_type)

# with open('cities_test', 'w') as file:
#         file.write(a.encode('utf-8'))

with open('locationsFB.csv', 'wb') as f:
